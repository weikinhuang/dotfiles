...
Important: only [`plugins/00-bash-opts.sh`](./plugins/00-bash-opts.sh) and
[`plugins/00-chpwd-hook.sh`](./plugins/00-chpwd-hook.sh) load by default. The rest of [`plugins/*.sh`](./plugins) load
only when `DOT_INCLUDE_BUILTIN_PLUGINS=1` is set before startup.
...
- [`plugins/90-termux.sh`](./plugins/90-termux.sh) -> `DOT_PLUGIN_DISABLE_termux=1`
...
- [`curlrc`](./curlrc), [`wgetrc`](./wgetrc), [`inputrc`](./inputrc), [`gitconfig`](./gitconfig), [`vimrc`](./vimrc),
  and similar top-level files are installed via [`bootstrap.sh`](./bootstrap.sh), so those defaults apply independently
  of shell plugin loading.
...
| `termux`                                 | [`plugins/90-termux.sh`](./plugins/90-termux.sh)                                                                                                                                                                                         | On Android Termux, exported `pbcopy`/`pbpaste`/`open`/`quick-toast` that override Linux PATH scripts; `keep-awake`, `termux-sshd`, and `sdcard`/`dl` aliases when `~/storage` exists |
...
| `pi`                                                                                    | [`plugins/30-pi.sh`](./plugins/30-pi.sh), `pi`                                                           | wrapper that accepts `-u <profile>` / `-u=<profile>` to switch to a named pi coding agent profile: creates `${XDG_CONFIG_HOME:-$HOME/.config}/pi-<profile>/` if missing, points `PI_CODING_AGENT_DIR` at it, exports `PI_CODING_AGENT_PROFILE_NAME=<profile>`, sources `<profile-dir>/env` if present, then execs the real `pi` in a subshell so env changes do not leak. Tab-completes profile names after `-u`. |
...
| `pbcopy`, `pbpaste`, `open`, `quick-toast`, `keep-awake`, `termux-sshd`, `sdcard`, `dl` | [`plugins/90-termux.sh`](./plugins/90-termux.sh), `termux-setup-storage`                                 | exported clipboard/open/notification helpers that override Linux PATH scripts; `keep-awake` wraps a command with `termux-wake-lock`; `termux-sshd` starts/stops/reports sshd on port 8022; `sdcard`/`dl` when `~/storage` exists                                                                                                                                                                                  |
...
| ------------------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`ai-cost-doctor`](./dotenv/bin/ai-cost-doctor)               | all               | cross-harness session cost / caching doctor; `ai-cost-doctor <harness> [id                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | prefix                                                                                                                                                                                                                                                                                                        | path]`(harness-first like`ai-tool-usage`, e.g. `ai-cost-doctor pi 019f0109`; omit the selector for your latest session, or pass a bare `.jsonl` path to auto-detect pi/claude/codex; opencode reads its SQLite DB by session id); reconstructs the per-turn token/cost series and flags cost explosions + prompt-caching pathologies (`cache-poisoning`, `cache-write-dominant`, `ttl-expiry`, `large-context-carry`) with the offending turn range, dollars attributed, and a remediation hint; `--json`for machine output,`--no-cost`/`--refresh-prices`control the LiteLLM cost backfill for logs lacking precomputed cost (claude/codex/opencode); the detection counterpart to the pi-only`cache-breakpoint` extension; delegates to [`lib/node/ai-tooling/session-doctor.ts`](./lib/node/ai-tooling/session-doctor.ts), see [`lib/node/ai-tooling/SESSION-DOCTOR.md`](./lib/node/ai-tooling/SESSION-DOCTOR.md) |
| [`ai-fetch-web`](./dotenv/bin/ai-fetch-web)                   | all               | harness-agnostic CLI for the [`fetch_web`](https://github.com/modelcontextprotocol/servers) MCP server (search, fetch, convert, extract, screenshot, ...); speaks MCP JSON-RPC over HTTPS, configured via `AI_FETCH_WEB_URL` + `AI_FETCH_WEB_AUTH`; pairs with [`config/pi/skills/ai-fetch-web/SKILL.md`](./config/pi/skills/ai-fetch-web/SKILL.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| [`ai-skill-eval`](./dotenv/bin/ai-skill-eval)                 | all               | harness-agnostic SKILL.md validation harness (`list` / `run` / `grade` / `rerun` / `report` / `validate` / `benchmark` / `optimize`); discovers SKILL.md files under `.agents/skills`, `config/agents/skills`, `config/pi/skills`, `.claude/skills`; drives a model (default `pi -p --model llama-cpp/qwen3-6-35b-a3b --no-session`, or `claude -p`, or `codex exec`, or `--driver-cmd` for a custom wrapper) against per-skill `evals/evals.json`; runs up to `--num-workers W` driver calls in parallel (default 1) and kills any single call exceeding `--timeout T` seconds (default 30, appending `DRIVER_TIMEOUT` to the run output and the grade's `flaws`); grades TRIGGER + expectation keyword-match with an optional `--critic-cmd` for subjective judgment; `validate` lints SKILL.md frontmatter (kebab-case name, ≤1024-char description with no `<` / `>`, ≤500-char compatibility, whitelisted top-level keys) and also runs as a pre-flight before `run` / `grade` / `rerun`; `benchmark` aggregates existing grades + per-run metrics sidecars into `benchmark.json` + `benchmark.md` per skill (mean/stddev/min/max for pass*rate + time_seconds + tokens, plus a first-minus-second delta column when `--baseline` was used); each `run` writes to a new `iteration-<N>/` subdir under the workspace so history is preserved, maintains a `latest` symlink, accepts `--iteration N` to overwrite an existing slot, and `report --compare-to M` emits a cross-iteration Δ table; `optimize <skill>` iteratively rewrites SKILL.md's `description:` frontmatter against a trigger-only eval set (default `<skill>/evals/trigger-evals.json`, fallback projection from `evals/evals.json`), splits the set with a stratified `--holdout` (default 0.4) train/test, calls the improver through the same driver abstraction with a blinded history (test*\* keys stripped) and a 1024-char safety-net rewrite, and prints the best-scoring description to stdout (pass `--write` to rewrite SKILL.md in place after snapshotting the previous description to `description-history.json` and printing a diff); pairs with [`config/agents/skills/ai-skill-eval/SKILL.md`](./config/agents/skills/ai-skill-eval/SKILL.md); TypeScript implementation lives under [`lib/node/ai-skill-eval/`](./lib/node/ai-skill-eval/) and `dotenv/bin/ai-skill-eval` is a thin shim |
...
These come from executables in [`dotenv/bin/`](./dotenv/bin) named `git-*`. In this repo,
[`git ignore`](./dotenv/bin/git-ignore) resolves to the `git-ignore` executable even though
[`config/git/alias.gitconfig.conf`](./config/git/alias.gitconfig.conf) also defines an `ignore` alias.
...
[`git-diff-highlight`](./dotenv/bin/git-diff-highlight) is also shipped in [`dotenv/bin/`](./dotenv/bin) and is used by
the default git pager configuration when `delta` is not active.
...
- `precmd` and `preexec` depend on [`external/bash-preexec.sh`](./external/bash-preexec.sh) and are skipped when
  `DOT_DISABLE_PREEXEC=1`.
- `chpwd` support comes from [`plugins/00-chpwd-hook.sh`](./plugins/00-chpwd-hook.sh), which is one of the two built-ins
  loaded by default.
...
This reference treats the following `internal::...` helpers as extension surface for local overrides. Other
`internal::...` functions are implementation detail.
...
The loader upgrades `TERM` to a 256-color variant for `xterm*`, `rxvt*`, and `screen*` sessions, and sets
`COLORTERM=truecolor` for terminals known to support 24-bit color (any `*-256color` variant, `alacritty`, `xterm-kitty`,
`xterm-ghostty`) when the variable is not already set by the terminal emulator.
...
| `DOT_DISABLE_EDITOR_AUTODETECT` | unset   | skip autodetecting VS Code, Cursor, or `npp` as the editor; falls through to `nvim`/`vim`. The in-terminal VS Code-family detection (via `TERM_PROGRAM=vscode` + `GIT_ASKPASS`) is unaffected, as is the existing SSH skip                                                                                                                            |
...
user-facing knobs in one place.

| Variable                          | Default | Effect                                                                                                                                                                                                                                                            |
| --------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PI_SANDBOX_DISABLED`             | unset   | bypass the sandbox extension entirely; bash subprocesses run unwrapped, statusline shows `🛡️·off`                                                                                                                                                                 |
| `PI_SANDBOX_DRY_RUN`              | unset   | log the wrapped command but pass the original through; for debugging the wrap pipeline                                                                                                                                                                            |
| `PI_SANDBOX_DEFAULT`              | `warn`  | fallback when `wrapWithSandbox` itself errors: `warn` (run unwrapped, log per-call), `allow` (run unwrapped silently), or `block` (refuse to run bash)                                                                                                            |
| `PI_SANDBOX_NETWORK_DEFAULT`      | `deny`  | non-UI default for the network ask-callback when a sandboxed bash hits a non-allowlisted domain in `pi -p` mode: `deny` (silent block) or `allow` (silent admit)                                                                                                  |
...
| `PI_SANDBOX_NETWORK_UNRESTRICTED` | unset   | set `network.unrestricted: true` for the session: drop kernel network isolation so the host network + `localhost` services (Docker published ports) are reachable, at the cost of NO domain filtering                                                             |
| `PI_SANDBOX_ALLOW_LOCALHOST`      | unset   | set `network.allowLocalhost: true` for the session: route loopback through the sandbox proxy so host `localhost` services are reachable while remote domain filtering stays on (HTTP/SOCKS clients only)                                                          |
| `PI_SANDBOX_ALLOW_ROOT`           | unset   | allow the sandbox extension to load when pi is running as root (containerised / privileged hosts where the threat model differs); off by default per plan section 6                                                                                               |
...
| `PI_MEMORY_DISABLE_CAPTURE`       | unset   | turn off the capture-assist nudge spliced into the turn after a compaction (lists concrete candidates mined from the compaction summary, else a generic reminder); also suppressed under `PI_MEMORY_READONLY`                                                     |
| `PI_MEMORY_CAPTURE_TURN`          | unset   | deliver the post-compaction capture directive as its own follow-up turn (`sendUserMessage`) instead of a `<system-reminder>` riding the next turn; small/weak models ignore the reminder but act on a dedicated turn, at the cost of one extra visible model turn |
...
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `AI_FETCH_WEB_URL`             | [`ai-fetch-web`](./dotenv/bin/ai-fetch-web)                                                                                          | MCP endpoint URL                                                                                                                          |
| `AI_FETCH_WEB_AUTH`            | [`ai-fetch-web`](./dotenv/bin/ai-fetch-web)                                                                                          | verbatim value for the `Authorization:` request header, e.g. `Basic abc...` or `Bearer sk-...`                                            |
| `AI_FETCH_WEB_HEADERS`         | [`ai-fetch-web`](./dotenv/bin/ai-fetch-web)                                                                                          | extra `Header: value` pairs, newline- or `;`-separated, appended to every MCP request                                                     |
| `AI_SKILL_EVAL_MODEL`          | [`ai-skill-eval`](./dotenv/bin/ai-skill-eval)                                                                                        | default model id passed to the driver when `--model` is unset (e.g. `llama-cpp/qwen3-6-35b-a3b` for pi, a claude model id for claude)     |
| `AI_SKILL_EVAL_WORKSPACE`      | [`ai-skill-eval`](./dotenv/bin/ai-skill-eval)                                                                                        | default workspace directory when `--workspace` is unset (default `.ai-skill-eval/` in cwd)                                                |
| `AI_SKILL_EVAL_DRIVER`         | [`ai-skill-eval`](./dotenv/bin/ai-skill-eval)                                                                                        | default driver when `--driver` is unset; `pi`, `claude`, or `codex` (auto-detected from `$PATH` if unset)                                 |
| `AI_SKILL_EVAL_PROMPT_FILE`    | [`ai-skill-eval`](./dotenv/bin/ai-skill-eval) driver / critic wrappers                                                               | set by the tool when invoking `--driver-cmd` / `--critic-cmd`; path to the prompt text file on disk                                       |
...
| `DOT_AI_FETCH_WEB_LIVE`        | [`tests/dotenv/bin/ai-fetch-web.bats`](./tests/dotenv/bin/ai-fetch-web.bats)                                                         | set to `1` to run live-server smoke tests (default: skipped, credential-free)                                                             |
...
- `nvm`: when `DOT_INCLUDE_BUILTIN_PLUGINS=1`, the nvm plugin lazy-loads `nvm.sh` on first use. The default node
  version's bin dir is cached in `~/.config/dotfiles/cache/nvm_default_path` so `node`/`npm`/`npx` are available
  immediately. If the cache is missing, nvm is sourced once to seed it.
- completions: with the full built-in plugin set enabled, completion scripts for tools like `kubectl`, `helm`, `gh`,
  `npm`, and `podman` can add up. The repo caches generated completions where possible.
- prompt segments: the load-average and exec-timer segments spawn subprocesses on older Bash setups. Remove them with
  `internal::ps1-segment-remove loadavg` or `internal::ps1-segment-remove exec_time`.
- git prompt status: if `__git_ps1` is still slow in very large repos, increase `DOT_GIT_PROMPT_CACHE_TTL_MS` or
  `DOT_GIT_PROMPT_CACHE_MAX_AGE_MS`.
- local overrides: `~/.bash_local`, `~/.bash_local.d/*.sh`, `~/.bash_local.d/*.plugin`, and `dotfiles_complete()` can
  add significant local startup cost.
...
The prompt uses UTF-8 symbols such as lambda, mu, pi, and the Armenian git marker. Your terminal and font must support
Unicode. Nerd Font variants such as `JetBrainsMono Nerd Font` or `FiraCode Nerd Font` usually work well.

### `date2unix` does not parse a date

On macOS, `date2unix` first tries GNU-style parsing with `date --date`, then `gdate` from Homebrew, then BSD
`date -j -f` fallbacks for a few common formats. Installing `coreutils` gives the broadest date parsing via `gdate`.

### Local overrides

Machine-specific configuration belongs in `~/.bash_local`, `~/.bash_local.d/*.sh`, and `~/.bash_local.d/*.plugin`. That
keeps the repo itself portable while still letting you change startup behavior, env vars, hooks, and plugin loading
locally.
...
[`clipboard-server`](./dotenv/bin/clipboard-server) forwards local clipboard access over an HTTP socket. It is primarily
intended for SSH sessions that want to use the local workstation clipboard remotely.
...
The HTTP API is unauthenticated by design. When `--socket` is a TCP port, the server binds to `127.0.0.1` only; always
reach it from another host through an SSH `-R` tunnel rather than exposing the port directly.
