...
| [`hooks-example.json`](./hooks-example.json)                       | Annotated starter for the [`hooks`](./extensions/hooks.md) extension. Copy to `~/.pi/hooks.json` or `<repo>/.pi/hooks.json`. Each event class (`PreToolUse`, `PostToolUse`, `UserPromptSubmit`) ships with one commented-out example.                                                                        |
