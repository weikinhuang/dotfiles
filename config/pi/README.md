...
| [`context-trim-example.json`](./context-trim-example.json)         | Threshold + caption-model config for the [`context-trim`](./extensions/context-trim.md) extension (`minTextBytes`, `snippetChars`, `captionModel`). Copy to `~/.pi/agent/context-trim.json` or `<repo>/.pi/context-trim.json`.                                                                                     |
...
| [`secret-redactor-example.json`](./secret-redactor-example.json)   | Layers / custom `rules` / `allowlist` / `keywordMinLength` config for the [`secret-redactor`](./extensions/secret-redactor.md) extension. Copy to `~/.pi/agent/secret-redactor.json` or `<repo>/.pi/secret-redactor.json`.                                                                                         |
| [`strip-reasoning-example.json`](./strip-reasoning-example.json)   | Model allowlist + `keepLast` for the [`strip-reasoning`](./extensions/strip-reasoning.md) extension. Copy to `~/.pi/agent/strip-reasoning.json` or `<repo>/.pi/strip-reasoning.json`.                                                                                                                              |
| [`tool-rescue-example.json`](./tool-rescue-example.json)           | Opt-in `tools` allowlist for the [`tool-rescue`](./extensions/tool-rescue.md) extension (HARD-DENY execute/mutate tools are never rescued). Copy to `~/.pi/agent/tool-rescue.json` or `<repo>/.pi/tool-rescue.json`.                                                                                               |
