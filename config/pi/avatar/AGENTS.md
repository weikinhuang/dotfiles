...
- **Local ComfyUI:** the standalone `tools/gen-comfyui.ts` drives a self-hosted ComfyUI directly with an
  edit-from-canonical (HERO-anchored) consistency strategy and an A/B/C model-comparison harness
  (`tools/compare-sheet.ts`, `tools/assemble-sheets.ts`, `tools/workflow-registry.ts`). Per-model graphs, the registry
  (`avatar-ref/workflows.json`), and model downloads are device-local. Both backends are documented in
  [`tools/PROMPTS.md`](./tools/PROMPTS.md).
