...
| `fetch <url>`                   | url                                             | `--format markdown\|html\|text\|readability\|summary\|chunks`, `--renderer http\|browser`, `--raw`, `--continue`, `--max-bytes N` | article body (markdown by default); response metadata stripped unless `--raw`. `--continue` follows continuation tokens until done or the cumulative `--max-bytes` cap (default 1 MiB) is reached; without `--continue`, a resume hint is printed to stderr when the body is truncated |
...
| `query <url>`                   | url + `--selector CSS`                          | `--attr text\|html\|<name>`, `--raw`                                                                                              | single selector match (text by default; `--attr html` or attribute name)                                                                                                                                                                                                               |
