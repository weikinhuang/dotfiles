...
- `complete` from `in_progress` requires a `note`; from `review` the note is optional (the review step was the parking).
  `block` and `cancel` always require a note.
...
1. A progress bar (`▰` filled / `▱` empty) sized to the terminal (8 cells at 80 cols, up to 20 at wide terminals)
   followed by the percentage chip and a count summary (`1 active · 1 review · 2 pending · 1 blocked · 2 cancelled` -
...
The `<from>` glyph is the per-action default from [`transitionGlyphs`](../../../lib/node/pi/todo-reducer.ts) rather than
the live previous status, because `renderCall` only has access to the tool args. `add` / `list` / `clear` omit the
triplet entirely.

`renderResult` emits the grouped layout: header line (`N/total done  ▰▰▰▱▱▱▱▱ · …`), then each non-empty section.
Collapsed view caps the `Completed` section behind a `… N completed (Ctrl+O to expand)` hint; `Cancelled` is uncapped in
v1 because the note carries the why-it-closed reason and that signal is worth seeing.
