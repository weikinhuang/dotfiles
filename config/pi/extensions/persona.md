...
(model swap) and [`filesystem.ts`](./filesystem.ts) (negative read/write gate): preset answers "which model", filesystem
answers "what's off-limits", persona answers "who am I right now and where am I allowed to write".

## What it does

Registers a `--persona` CLI flag, a `/persona` command (with `off`, `info <name>`, and `opener [n]` subcommands), and a
`Ctrl+Shift+M` cycle shortcut. When a persona is activated it snapshots the current model / thinking level / active
tools, applies the persona's overrides via `pi.setActiveTools` + `pi.setModel` + `pi.setThinkingLevel`, and wires up a
`before_agent_start` hook to append the persona body (and any `appendSystemPrompt`) to the system prompt. Activation is
persisted via a `customType: 'persona-state'` session entry so `/resume` re-applies on reload; `/persona off` (or
cycling past the last persona) restores the pre-persona snapshot. Status badge `persona:<name>` reflects the active
persona.
...
| `roleplay`             | `boolean?`                             | **Persona-only.** Opt-in master switch for the [`roleplay.ts`](./roleplay.md) extension. When `true`, the `roleplay` tool, cast scan, and `## Roleplay` injection activate; otherwise roleplay stays dormant.                                                                                                                          |
| `cast`                 | `string?`                              | **Persona-only.** Cast slug for the roleplay store when `roleplay: true`. Defaults to the persona name. Ignored unless `roleplay` is set.                                                                                                                                                                                              |
| `avatarSet`            | `string?`                              | **Persona-only.** Avatar sprite-set name for the reactive avatar, decoupled from `cast`. Overrides the default set resolution (first non-POV character → first character → cast slug). Use when the cast slug has no `avatar/emotes/<slug>/` dir, e.g. `cast: exusiai-isekai` + `avatarSet: exusiai`. Ignored unless `roleplay: true`. |
| `characters`           | `string[]?`                            | **Persona-only.** Cast members whose full sheets fold into the `## Roleplay scene` block, in order (inline array). Unresolved names are warn-dropped. Ignored unless `roleplay: true`. See [`roleplay.md`](./roleplay.md).                                                                                                             |
| `pov`                  | `string?`                              | **Persona-only.** The character the human plays; announced + folded last in the scene block. May name an off-cast character. Ignored unless `roleplay: true`.                                                                                                                                                                          |
| `openers`              | `string[]?`                            | **Persona-only.** Greeting lines surfaced via `/persona opener [n]` (inline array). Not injected into the prompt.                                                                                                                                                                                                                      |
| `authorNote`           | `string?`                              | **Persona-only.** Standing instruction the [`roleplay.ts`](./roleplay.md) extension injects at conversational depth (not the system prompt) via the `context` event. Inert unless `roleplay: true`.                                                                                                                                    |
| `authorNoteDepth`      | `number?`                              | **Persona-only.** Depth (messages from the end) at which `authorNote` is inserted. Default 4. Ignored unless `authorNote` is set.                                                                                                                                                                                                      |
...
helper [`filesystem.ts`](./filesystem.ts) uses, offering three options:
...
treated as its link path, not its target, mirroring `filesystem.ts`'s convention. If `plans/leak` is a symlink to
