...
import { renumber, type CitationSource } from './citations.ts';
import { appendJournal } from './journal.ts';
import { labRoot, paths, runRoot } from './paths.ts';
import { writePlan, type AutoresearchPlan, type DeepResearchPlan, type PlanBudget } from './plan.ts';
import {
  fetchAndStore,
  type McpClient,
...
function fixtureExpectedDir(flow: 'deep-research' | 'autoresearch'): string {
  ...
  // fixture dir is the sibling `selftest-fixtures/` directory. We
  ...
  return resolve(here, 'selftest-fixtures', flow, 'expected');
}
