...
import { type PlanningCriticOutcome, type PlanningCriticRunner, runPlanningCritic } from './planning-critic.ts';
import { writeRubricFiles } from './rubric.ts';
import { runSelfCritic } from './self-critic.ts';
import { type PhaseEvent } from './statusline.ts';
import { type SynthMergeResult, UnknownPlaceholderError } from './synth-merge.ts';
import { type SectionOutcome } from './synth-sections.ts';
import { populateSourceStore, runSynthPhase } from './synth-pipeline.ts';
import {
  type FanoutDeps,
  type FanoutResult,
  type FanoutSpawner,
