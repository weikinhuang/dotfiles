...
export interface WriteGateOptions {
  ...
  hasUI: boolean;
  /** `PI_PERSONA_VIOLATION_DEFAULT` resolved value, only consulted when `!hasUI`. */
...
export function decideWriteGate(opts: WriteGateOptions): WriteGateDecision {
  const { absolutePath, inputPath, resolvedWriteRoots, sessionAllow, hasUI, violationDefault, personaName } = opts;
  ...
  const detail =
    resolvedWriteRoots.length === 0
      ? `persona "${personaName}" disallows writes`
      : `persona "${personaName}" writeRoots: ${resolvedWriteRoots.join(', ')}`;
  ...
  return {
    ...
    reason:
      ...
      'Set PI_PERSONA_VIOLATION_DEFAULT=allow to override, or pick a path under writeRoots.',
  };
}
