 ...
 */
export function loadPersonaSettings(layers: SettingsLayer[]): LoadPersonaSettingsResult {
  const warnings: PersonaWarning[] = [];
  const merged: PersonaSettings = { writeRoots: {} };
  ...
  for (const layer of layers) {
    ...
    if (obj.writeRoots !== undefined) {
      if (obj.writeRoots === null || typeof obj.writeRoots !== 'object' || Array.isArray(obj.writeRoots)) {
        ...
        for (const [personaName, value] of Object.entries(wr)) {
          if (!isStringArray(value)) {
            warnings.push({
              path: layer.source,
              reason: `\`writeRoots.${personaName}\` must be an array of strings (dropped)`,
            });
            continue;
          }
          merged.writeRoots[personaName] = [...value];
        }
      }
    }
