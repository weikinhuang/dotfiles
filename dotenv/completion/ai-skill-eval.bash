...
_dot_ai_skill_eval() {
  ...
  if [[ -z "${op}" ]]; then
    if [[ "${cur}" == -* ]]; then
      ...
      mapfile -t COMPREPLY < <(compgen -W "list run grade rerun report validate benchmark compare analyze optimize" -- "${cur}")
    fi
    return
  fi
  ...
  if [[ "${cur}" == -* ]]; then
    ...
    case "${op}" in
      ...
      analyze) flags="${flags} --iterations --only" ;;
  ...
  case "${op}" in
    run | grade | report | validate | benchmark | compare | analyze | optimize)
      local names=""
