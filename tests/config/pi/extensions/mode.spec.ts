...
interface FakeApiState {
  ...
  thinkingLevel: ModeThinkingLevel | undefined;
  activeTools: string[];
}
 ...
 * after `parseModeFile` runs against the shipped catalog. Only the
...
test('plan #3: `write` to path outside `writeRoots` triggers a prompt with mode name + roots in detail', () => {
  ...
  expect(decision.detail).toContain('mode "plan"');
...
// `PI_MODE_VIOLATION_DEFAULT=allow` flips the default.
// ──────────────────────────────────────────────────────────────────────

test('plan #6: no-UI + default `deny` → block with PI_MODE_VIOLATION_DEFAULT hint', () => {
  const decision = decideWriteGate({
    absolutePath: '/repo/src/foo.ts',
    inputPath: 'src/foo.ts',
    resolvedWriteRoots: ['/repo/plans/'],
  ...
  expect(decision.reason).toContain('PI_MODE_VIOLATION_DEFAULT=allow');
});

test('plan #6: no-UI + `PI_MODE_VIOLATION_DEFAULT=allow` → allow (override)', () => {
  const decision = decideWriteGate({
    absolutePath: '/repo/src/foo.ts',
    inputPath: 'src/foo.ts',
    resolvedWriteRoots: ['/repo/plans/'],
