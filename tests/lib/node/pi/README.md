...
- **Lib code is pure.** `lib/node/pi/*.ts` imports only from `node:*` — never from
  `@mariozechner/pi-coding-agent`. That keeps tests hermetic, lets the helpers be reused across extensions without
