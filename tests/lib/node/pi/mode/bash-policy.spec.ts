...
import { evaluateBashPolicy, matchBashPattern } from '../../../../../lib/node/pi/mode/bash-policy.ts';
...
test('evaluateBashPolicy: empty allow + empty deny → allow (mode has no opinion)', () => {
  expect(evaluateBashPolicy({ command: 'ls', bashAllow: [], bashDeny: [], modeName: 'debug' })).toEqual({
    kind: 'allow',
  });
});
...
test('evaluateBashPolicy: deny `*` blocks any command (catalog plan/journal/roleplay/review)', () => {
  ...
  expect(result.reason).toContain('mode "plan"');
...
test('evaluateBashPolicy: non-empty allow, head misses → block with allow-list in reason', () => {
  ...
  expect(result.reason).toContain('mode "chat"');
